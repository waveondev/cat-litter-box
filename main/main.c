...
#include "esp_bt.h"
#include "nimble/nimble_port.h" // NimBLE 사용 시
...
static void vTaskListCustom(void)
{
    ...
    if (pxTaskStatusArray != NULL) {
        ...
        for (UBaseType_t x = 0; x < uxArraySize; x++) {
            ...
            // Core ID ���ڿ� ��ȯ (���� �� �� = Any, ������ = Core 0 �Ǵ� 1)
            char core_str[32]; // 32����Ʈ�� �˳��ϰ� ����
...
static int uart_parser(message_t *msg)
{
    ...
    while(RingBuffer_Pop(&ch)) 
    {
        ...
        if (parse_idx < PARSE_BUF_SIZE - 1) {
            ...
            // ���� ���� '\n' ���� �� ���ɾ� ó��
            if (ch == '\r'|| ch == '\n') {
                parse_buf[parse_idx] = '\0'; // ���ڿ� ���� ó��
                Process_Command(msg, parse_buf);  // ó�� �Լ� ȣ��
                parse_idx = 0;               // �ļ� ���� �ʱ�ȭ
            }
        } else {
            // ���� �����÷ο� ������ ���� �ʱ�ȭ
...
//	Usage();
    ...
    while(1)
    {
        ...
        if(get_aws_started())
        {
            ...
            ESP_LOGI(TAG, "프로비저닝 완전 종료 완료. 확보된 Free Heap: %d", xPortGetFreeHeapSize());
    ...
    while(1)
    {
        ...
        {
            g_start_ota_flag = false; // 중복 실행 방지
            
            ESP_LOGI(TAG, "==== NimBLE 안전 종료 및 메모리 해제 시작 ====");

            // 1. NimBLE 호스트 스택 중지 (스마트폰과의 연결 및 Notify 작업을 안전하게 정리)
