...
void write_nvs_memory(const char* name, const char* key, const char* data)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "[%s] NVS Open failed (%s)", name, esp_err_to_name(err));
        return;
    }
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "[%s -> %s] saved success: %s", name, key, data);
    } else {
        ESP_LOGE(TAG, "[%s -> %s] failed to save (%s)", name, key, esp_err_to_name(err));
    }

    err = nvs_commit(my_handle);
    if (err != ESP_OK) ESP_LOGE(TAG, "failed to commit!");
...
esp_err_t read_nvs_memory(const char* name, const char* key, char* out_data, uint16_t max_len)
{
    ...
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        ESP_LOGW(TAG, "[%s -> %s] no saved data.", name, key);
        nvs_close(my_handle);
        return err;
    }
    ...
    if (err == ESP_OK && required_size > 0) {
        ...
        if (required_size > max_len) {
            ESP_LOGE(TAG, "not enough buffer. (request : %d, acceptable: %d)", required_size, max_len);
            nvs_close(my_handle);
        ...
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "[%s -> %s] read success: %s", name, key, out_data);
        } else {
            ESP_LOGE(TAG, "[%s -> %s] failed to read (%s)", name, key, esp_err_to_name(err));
        }
    }
...
void write_nvs_int(const char* name, const char* key, int32_t value)
{
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "[%s -> %s] int save successs: %ld", name, key, value);
    } else {
        ESP_LOGE(TAG, "[%s -> %s] int save failed (%s)", name, key, esp_err_to_name(err));
    }
...
int32_t read_nvs_int(const char* name, const char* key, int32_t default_value)
{
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "[%s -> %s] int read success: %ld", name, key, out_value);
    } else if (err == ESP_ERR_NVS_NOT_FOUND) {
        ESP_LOGW(TAG, "[%s -> %s] no saved int, use default (%ld).", name, key, default_value);
    } else {
        ESP_LOGE(TAG, "[%s -> %s] int read failed (%s)", name, key, esp_err_to_name(err));
    }
...
void write_nvs_blob(const char* name, const char* key, const void* blob_data, size_t length)
{
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "[%s -> %s] blob save success (%d 바이트)", name, key, length);
    } else {
        ESP_LOGE(TAG, "[%s -> %s] blot save failed (%s)", name, key, esp_err_to_name(err));
    }
...
esp_err_t read_nvs_blob(const char* name, const char* key, void* out_blob, size_t max_length)
{
    ...
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        ESP_LOGW(TAG, "[%s -> %s] no saved blob.", name, key);
        nvs_close(my_handle);
        return err;
    }
    ...
    if (err == ESP_OK && required_size > 0) {
        ...
        if (required_size > max_length) {
            ESP_LOGE(TAG, "Not enough blob buffer (request: %d, acceptable: %d)", required_size, max_length);
            nvs_close(my_handle);
            return ESP_ERR_INVALID_SIZE;
        }
        ...
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "[%s -> %s] blob read success (%d bytes)", name, key, required_size);
        } else {
            ESP_LOGE(TAG, "[%s -> %s] blob read failed (%s)", name, key, esp_err_to_name(err));
        }
    }
...
void delete_nvs_key(const char* name, const char* key)
{
    ...
    if (err == ESP_OK) {
        ESP_LOGI("NVS", "[%s] Key delete success", key);
    } else if (err == ESP_ERR_NVS_NOT_FOUND) {
        ESP_LOGW("NVS", "[%s] try to delete, but key was not exist", key);
    }
...
void delete_nvs_namespace(const char* name)
{
    ...
    if (err == ESP_OK) {
        ESP_LOGI("NVS", "[%s] namespace overall delete success (init OK!)", name);
    }
