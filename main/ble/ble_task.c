...
//        MotionSetTimer(is_phone_connected);
    ...
    case BLE_GAP_EVENT_DISC:
    {
       ...
       rc = ble_hs_adv_parse_fields(&fields, event->disc.data, event->disc.length_data);
        ...
        // -----------------------------------------------------------------
        // 1. ?? [새로운 필터] RSSI 조건 검사 (간혹 부호가 헷갈릴 수 있으니 주의)
     ...
     //       return 0; // -55보다 큰 신호는 여기서 즉시 차단
        ...
        printf("\n [Wave_Tracker] satisfying condition dev_list saved OK! \n");
        ...
        if (actual_data_len > 0) {
            ...
            for (int i = 0; i < actual_data_len; i++) {
                // ?? 64바이트 버퍼가 가득 차는 것을 방지 (마지막 널 문자를 위해 62번 인덱스까지만 저장)
            ...
            printf("HEX  : empty service data payload \n");
        }
        printf("-------------------------------------\n");
...
bool ble_send_data_to_queue(const uint8_t *data, uint16_t len)
{
    ...
    if (ble_tx_queue == NULL) {
        printf("[BLE TX FUNC] 에러: ble_tx_queue was not initialized yet\n");
        return false;
    }
    ...
    if (len > 128) {
        my_packet.len = 128;
        printf("[BLE TX FUNC] warn : exceed 128 bytes data, truncated \n");
    } else {
        my_packet.len = len;
    }
    ...
    if (xQueueSend(ble_tx_queue, &my_packet, 0) == pdTRUE) {
        ...
        printf("[BLE TX FUNC] error : transmit queue was full, unable to push data\n");
        return false;
    }
}
...
{
    ble_data_msg_t msg;
    MODLOG_DFLT(INFO, "user data process task start\n");
...
{
    ble_data_msg_t msg;
    MODLOG_DFLT(INFO, "BLE transmit task start\n");
    ...
    while (1) {
        ...
        if (xQueueReceive(ble_tx_queue, &msg, portMAX_DELAY) == pdTRUE) {
            if (is_phone_connected && current_conn_handle != BLE_HS_CONN_HANDLE_NONE) {
                ...
                printf("[TX task] to mobile %d bytes Notify transmit finished \n", msg.len);
            } else {
                printf("[TX 태스크] warn : cancel transmit because mobile not connected \n");
            }
        }
    }
}
