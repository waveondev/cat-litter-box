...
void Tracker_In_ID(char* Tracker_ID)
{
    ...
    for (int i = 0; i < TRACKER_DEVICE_MAX; i++) {
        ...
        if (Tracker_Device[i] != NULL) {
            if (strcmp(Tracker_Device[i]->Device_ID, Tracker_ID) == 0) {
                ...
                printf("[Alarm] device create failed : ID [%s] is exist in %d slot already \n", Tracker_ID, i);
                return; 
            }
        }
    ...
    if (target_index == -1) {
        printf("[Error] device create failed: full storage space! (MAX: %d)\n", TRACKER_DEVICE_MAX);
        return; 
    }
...
void vTrackerCaptureTask(void *pvParameters)
{
    ...
    printf("tracker_capture task start. (period : 100ms)\n");
